...
const sorting = (number) => data.species.reduce((acc, specie) => ({
  ...acc,
  [specie.location]: data.species.map((num) => {
    if (num.location !== specie.location) return null;
    if (number.sex) {
      return {
        [num.name]: num.residents.filter((element) => element.sex === number.sex)
...
const nameSearch = (arg) => data.species.reduce((acc, specie) => ({
  ...acc,
  [specie.location]: data.species.map((numb) => {
    if (numb.location !== specie.location) return null;
    if (arg.sex) {
      return {
        [numb.name]: numb.residents.filter((element) => element.sex === arg.sex)
...
const namesByLocation = data.species.reduce((acc, specie) => ({
  ...acc,
